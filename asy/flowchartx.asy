...
block parallelogram(object body, pair center=(0,0), pen fillpen=invisible,
	    pen drawpen=currentpen, real dh=2,
	    real dw=5, real minwidth=0, real minheight=0)
{
  frame f=body.f;
  pair m=min(f);
  ...
  path shape=(1.5dw+a,b+dh)--(2dw+a,2b+2dh)--(dw,2b+2dh)--(0.5dw,b+dh)--(0,0)--
    (dw+a,0)--cycle;
  block block;
  block.draw=new frame(pen p) {
    frame block;
  ...
  block.size=(2dw+a,2dh+2b);
  block.f_bottomleft=point(shape,4); 
  block.f_bottom=point(shape,5)-(0.5a,0); 
  block.f_bottomright=point(shape,5); 
  block.f_right=point(shape,0); 
  block.f_topright=point(shape,1); 
  block.f_top=point(shape,2)+(0.5a,0); 
  block.f_topleft=point(shape,2); 
  block.f_left=point(shape,3); 
  return block;
}
